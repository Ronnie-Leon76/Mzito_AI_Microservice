...
On Azure App Service / Container Apps, add the same three variables under **Application settings** / **Environment variables** in the portal (or your IaC template). On **Render**, set `WEBHOOK_SHARED_SECRET` under **Environment** to the **exact same value** as `WEBHOOK_SECRET` in the Deluge script.
